...
function toggleAuth(show){
  document.documentElement.classList.toggle('auth-locked', show);
  ...
  if(show){
    ...
    els.authGate.style.visibility = '';
    els.authGate.style.pointerEvents = '';
    els.authGate.removeAttribute('aria-hidden');
  }else{
    els.authGate.style.display = 'none';
    els.authGate.style.visibility = 'hidden';
    els.authGate.style.pointerEvents = 'none';
...
async function applySession(user){
  ...
  window.TOPBRS_APP?.render?.();
  setTimeout(()=>toggleAuth(false), 120);
  setTimeout(()=>window.TOPBRS_APP?.render?.(), 220);
...
els.loginForm?.addEventListener('submit', async (e)=>{
  ...
  try{
    ...
    toggleAuth(false);
    setTimeout(()=>toggleAuth(false), 100);
    setTimeout(()=>window.TOPBRS_APP?.render?.(), 120);
  }catch(err){
    setLoginButtonLoading(false);
    toggleLoader(false);
...
// ===== V2.4.7 AUTH GATE HIDDEN DEFINITIVO =====
function forceHideAuthGateDefinitive(){
  if(!els.authGate) return;
...
window.forceHideAuthGateDefinitive = forceHideAuthGateDefinitive;
window.addEventListener('load', ()=>{ if(auth.currentUser) forceHideAuthGateDefinitive(); });
setTimeout(()=>{ if(auth.currentUser) forceHideAuthGateDefinitive(); }, 400);
setTimeout(()=>{ if(auth.currentUser) forceHideAuthGateDefinitive(); }, 1200);


// ===== V2.4.8 AUTH UI STABLE HIDE =====
(function(){
  function hideGateStable(){
    if(!els.authGate) return;
    els.authGate.classList.add('hidden');
    els.authGate.style.display = 'none';
    els.authGate.style.visibility = 'hidden';
  ...
  const originalToggleAuth = toggleAuth;
  toggleAuth = function(show){
    originalToggleAuth(show);
    if(!show) hideGateStable();
  };

  window.addEventListener('load', ()=>{ if(auth.currentUser) hideGateStable(); });
  setTimeout(()=>{ if(auth.currentUser) hideGateStable(); }, 300);
  setTimeout(()=>{ if(auth.currentUser) hideGateStable(); }, 1000);
})();
